...
import type { Hero, HeroStats } from "../../global-types/types";
import type { HeroLore } from "./types";
...
function heroFilteredStatsInfo(data: any, id: string) {
  const filteredHero = data.filter((hero: Hero) => hero.id === Number(id));
  return filteredHero
}

function heroLoreFilter(data: HeroLore, heroName: string) {
  const map = new Map();
...
function HeroPage() {
  const [heroData, setHeroData] = useState<HeroStats>();
  const [heroLore, setHeroLore] = useState();
  const { heroId, heroName }: any = useLoaderData();
    ...
    fetch(`https://api.opendota.com/api/heroStats`)
      ...
      .then(res => {
        setHeroData(prevState => {
          const [hero] = heroFilteredStatsInfo(res, heroId)
          prevState = hero;
          return prevState
        })
      })
  ...
  useEffect(() => {
    ...
    .then(res => res.json())
    .then(res => setHeroLore(heroLoreFilter(res, heroName)))
  }, [])
      ...
      <article className="mt-16">
        <section className="hero">
          <div className="container">
            ...
            {heroLore && (
              <div className="hero-lore mt-12">
              <h2 className="text-3xl mb-4">Lore</h2>
              <p className="text-xl">
                {heroLore}
              </p>
            </div>
